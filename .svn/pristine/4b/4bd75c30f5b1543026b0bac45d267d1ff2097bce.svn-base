...
<body>
					...
					<div class="panel-body" id="searchOfForm" style="display: block;">
						<form class="form-inline" role="form" id="searchForm" action="<%=basePath%>admin/getEvidence.php" method="post">
							...
							<div class="form-group" style="width:22%">
								<label for="" class="clabel">关键字:</label>
								<input id="evName" name="evName" value="" 
									class="form-control" placeholder="请输入文件名或其它关键字..." type="text" style="width:60%" onkeydown="onKeyDown(event)"/>
							</div>
							<div class="form-group" style="width:22%">
								<label for="" class="clabel">案件名:</label>
								<input id="caseName" name="caseName" value=""
									class="form-control" placeholder="请输入案件名..." type="text" style="width:60%" onkeydown="onKeyDown(event)"/>
							</div>
							<div class="form-group">
								<label for="" class="clabel">类型:</label>
								<select class="form-control" id="evStatus" name="evStatus" onkeydown="onKeyDown(event)">
								    <option>全部 </option>
								    <option>.doc</option>
								    <option>.docx</option>
						            <option>.pdf</option>
					...
					<div class="panel-body" style="padding: 0 0 15px;">
						<table id="datatables" class="table table-striped table-hover br04"
							...
							<thead>
								<tr>
									...
									<th style="text-align:left;">协同分析</th>
									...
									<th style="text-align:left;">导入时间</th>
									<th style="text-align:left;">所属案件</th>
						...
						<div style="text-align:center;" style="font-size: 14px">
							...
							<div style="float: right;margin-right: 11px;">
								...
								<button type="button" class="btn" onclick="showAllHandledFile(1)" style="width:52px;height:28px;line-height: 12px;">跳转</button>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="modal-content">
					<div class="panel panel-default" style="padding-right: 10px;">
						<div class="panel-body" style="height: 950px;" >
							<div class="panel-heading br004 b29"
								...
								<div class="panel-heading br004 b22" style="padding: 10px;">
								   ...
								   <div style="margin-top: 17px;margin-left: 65px;"><span id="docfileNameyl"></span>&nbsp;&nbsp;&nbsp;<span id="docdateyl"></span><br><span id="docfileSizeyl" ></span>
								    <div style=" padding-left: 300px; padding-bottom: 0px;">
						                  <img class="b22" src="<%=basePath %>template/img/201.png" style="width: 15px;height: 15px;"><a id="url2" href="#"><span id="url2" onclick="downloaddoc()">下载文件</span></a>
						                   <span id="docfileurl" style=" display:none;"></span>
						                   <span id="docTypeyl" style=" display:none;"></span>
								          ...
								          <a href="#"  onclick="lookOnline()">在线预览</a>
								         </div> 
								         </div>
								<hr>
	...
	<!-- 弹出收藏成功框 -->
	<div class="modal fade" id="favoriteDoc" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
	...
	<!-- 弹出选择类别框 -->
	<div class="modal fade" id="pleaseChooseType" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				...
				<div style="margin-left: 3%;margin-top: 3%;">
					<h5 class="modal-title" >请先选择标签！</h5>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">确定</button>
				</div>
...
<script src="<%=basePath%>template/js/noTrim.js"></script>
...
function searchone(i) {
	$.ajax({
		...
		var elasticsearchId = document.getElementById("docdetails0"+i+"").innerText;
		...
		if("0"==xb){
		 ...
		 if(docfileSize/1024>1) {
			 fileSize_rignht=(docfileSize/1024).toFixed(2)+"MB";
		 }else{
			 fileSize_rignht=docfileSize+"KB";
		 }
		//es  id
		document.getElementById("essssid_right").innerText= elasticsearchId;
		document.getElementById("docPreviewId").innerText= docPreviewId;
		//表格中字段
		document.getElementById("id_right").innerText= xb;
		document.getElementById("caseNum_right").innerText= no_trim(docfileName);
		document.getElementById("caseName_right").innerText= docType;

		document.getElementById("caseType_rignt").innerText= fileSize_rignht;
		...
		document.getElementById("docfileNameyl").innerText= no_trim(docfileName);
		...
		var fileSize_rignht_tou;
		
		 if(docfileSize/1024>1) {
			 fileSize_rignht_tou=(docfileSize/1024).toFixed(2)+"MB";
		 }else{
			 fileSize_rignht_tou=docfileSize+"KB";
		 }
		 
		document.getElementById("docfileSizeyl").innerText= fileSize_rignht_tou;
...
function exportDoc() {
	...
	if(urlstr==""){
		...
		if (rs) {
			 //对lookdocpath进行编码
			var lookdocpath_encode=encodeURI(encodeURI(urlstr));
			$("#url").attr("href","<%=basePath %>admin/exportDOC.php?docpath="+lookdocpath_encode);
        } else {
...
//                var getContent = content.split("\n");
			...
			/* 	var docss=docs.size;
				...
				if(caseName!=null&&caseName!=""){
					...
					myCaseName=myCaseName.replace(eval(str),"<font class='c11'>"+str2+"</font>");
				}
				...
				if(evStatus!=null&&evStatus!=""){
					...
					dType=dType.replace(eval(str),"<font class='c11'>"+str2+"</font>");
				}
				...
				var html = '+<tr>'+
				    '<td style="text-align:left;"><input type="checkbox" id="caselist'+i+'" name="ids_url" value="'+item._source.file_download_url+'" onclick="showChoose(this.id);"/></td>'+
				    '<td style="text-align:left;"><img src='+src+' onclick="updocumentstar('+i+');" id="docstar'+i+'" data-target="#myModalfavorite" data-toggle="modal" /></td>'+
				    '<td style="text-align:left;"><img src='+readRrc+' id="ifRead'+i+'"/></td>'+
				    '<td style="text-align:left;" data-toggle="modal" data-target="#myModal" onclick="searchone('+i+')">'+no_trim(fileName)+'</td> '+
					'<td style="text-align:left;" data-toggle="modal" data-target="#myModal" onclick="searchone('+i+')">'+dType+'</td>'+
					'<td style="text-align:left;" data-toggle="modal" data-target="#myModal" onclick="searchone('+i+')">'+fileSize +'</td>'+
					'<td style="text-align:left;" data-toggle="modal" data-target="#myModal" onclick="searchone('+i+')">'+item._source.createDate +'</td>'+
					'<td style="text-align:left;" data-toggle="modal" data-target="#myModal" onclick="searchone('+i+')">'+item._source.editDate +'</td>'+
					'<td style="text-align:left;" data-toggle="modal" data-target="#myModal" onclick="searchone('+i+')">'+item._source.viewDate +'</td>'+
//					'<td style="text-align:left;" style=" display:none;">'+item._source.file_download_url +'</td>'+
					'<td style="text-align:left;" data-toggle="modal" data-target="#myModal" onclick="searchone('+i+')">'+item._source.importDate+'</td>'+
					'<td style="text-align:left;" data-toggle="modal" data-target="#myModal" onclick="searchone('+i+')">'+myCaseName+'</td>'+
...
//文档预览    
	function lookOnline(i) {
		$("#loadingDiv").show();
		var lookOnlinePath = document.getElementById("docfileurl").innerText;
		var docType = document.getElementById("caseName_right").innerText;
		
		if('.txt' == docType) {
		var esDocId = document.getElementById("essssid_right").innerText;
		$.ajax({
			type : "POST",
				...
				if(i % 7 == 1) {
					var html = '<span>'+
						'<span class="td_left"><input id="caselist'+i+'" type="checkbox"  name="ids" value="'+data[i].label+'" onclick="showChoose(this.id);" />'+
							'<span class="br05 b12 span-1" id="" style="margin-left:20px;padding-left:25px;padding-right:25px;">'+data[i].label+'</span>'+
						'</span><br/>';
						}
					if(i % 7 == 2) {
					var html = '<span>'+
						'<span class="td_left"><input id="caselist'+i+'" type="checkbox"  name="ids" value="'+data[i].label+'" onclick="showChoose(this.id);" />'+
							'<span class="b13 br06 span-2" id="" style="margin-left:20px;padding-left:25px;padding-right:25px;">'+data[i].label+'</span>'+
						'</span><br/>';
						}
					if(i % 7 == 3) {
					var html = '<span>'+
						'<span class="td_left"><input id="caselist'+i+'" type="checkbox"  name="ids" value="'+data[i].label+'" onclick="showChoose(this.id);" />'+
							'<span class="br11 b11 span-3" id="" style="margin-left:20px;padding-left:25px;padding-right:25px;">'+data[i].label+'</span>'+
						'</span><br/>';
						}
					if(i % 7 == 4) {
					var html = '<span>'+
						'<span class="td_left"><input id="caselist'+i+'" type="checkbox"  name="ids" value="'+data[i].label+'" onclick="showChoose(this.id);" />'+
							'<span class="br07 b14 span-4" id="" style="margin-left:20px;padding-left:25px;padding-right:25px;">'+data[i].label+'</span>'+
						'</span><br/>';
						}
					if(i % 7 == 5) {
					var html = '<span>'+
						'<span class="td_left"><input id="caselist'+i+'" type="checkbox"  name="ids" value="'+data[i].label+'" onclick="showChoose(this.id);" />'+
							'<span class="br08 b15 span-5" id="" style="margin-left:20px;padding-left:25px;padding-right:25px;">'+data[i].label+'</span>'+
						'</span><br/>';
						}
					if(i % 7 == 6) {
					var html = '<span>'+
						'<span class="td_left"><input id="caselist'+i+'" type="checkbox"  name="ids" value="'+data[i].label+'" onclick="showChoose(this.id);" />'+
							'<span class="br09 b17 span-6" id="" style="margin-left:20px;padding-left:25px;padding-right:25px;">'+data[i].label+'</span>'+
						'</span><br/>';
						}
					if(i % 7 == 7) {
					var html = '<span>'+
						'<span class="td_left"><input id="caselist'+i+'" type="checkbox"  name="ids" value="'+data[i].label+'" onclick="showChoose(this.id);" />'+
							'<span class="br10  b18 span-7" id="" style="margin-left:20px;padding-left:25px;padding-right:25px;">'+data[i].label+'</span>'+
						'</span><br/>';
						}
					if(i % 7 == 0) {
					var html = '<span>'+
