...
<script type="text/javascript">  
	...
	function init_for_ipMap(){
				...
				map.clearOverlays();//清空原来的标注				
	 		 	//map.centerAndZoom(new BMap.Point(93.39385,35.179228), 12);
		  		...
		  		//var marker = new BMap.Marker(new BMap.Point(93.39385,35.179228));  // 创建标注
...
//		document.getElementById("dasousuo").innerText=lac;
	...
	map2.centerAndZoom(point2,12);  // 初始化地图,设置中心点坐标和地图级别
	//map.addControl(new BMap.MapTypeControl());   //添加地图类型控件 离线只支持电子地图，卫星/三维不支持
	//map.setCurrentCity("北京");          // 设置地图显示的城市 离线地图不支持！！
	...
	function init_for_Map(){
			map2.clearOverlays();//清空原来的标注				
	 		//map2.centerAndZoom(new BMap.Point(93.39385,35.179228), 12);
			
	  			var test2 = jizhanInfo.toString();
...
//	  	        alert("lon2="+lon2);
		  		...
		  		point2 = new BMap.Point(lon2,lat2);
		  		map2.centerAndZoom(point2, 12);
		  		var marker2 = new BMap.Marker(point2);  // 创建标注
		  		//var marker2 = new BMap.Marker(new BMap.Point(93.39385,35.179228));  // 创建标注
...
//	window.onload = SearchGHDB(1);
	$(function(){
		if(typess==2){
				...
				$("#IPSearch").css("display","none");
				$("#jiZhan").css("display","none");
			
			
			 //SearchGHDB(1);
			 //alert(typess);
			 document.getElementById("ghdb").click();
			/*  document.getElementById("ghdb").onclick = f;
			 function f() {
				 document.getElementById("ghdb").onclick ="";
			} */
		};
		
		if(typess==3){
			$("#ghdb").css("display","none");
			$("#jiZhan").css("display","none");
			 document.getElementById("IPSearch").click();
			/* document.getElementById("IPSearch").onclick = e;
			function e() {
				document.getElementById("IPSearch").onclick ="";
			} */
		};
		...
		if(typess==4){
			...
			/* document.getElementById("jiZhan").onclick = a;
			function a() {
				document.getElementById("jiZhan").onclick ="";
			} */
		};
	});
...
//				$("#pageNos").css("display" , "none");
				...
				'<th class="menuss" style="text-align: center; width :10%;" >邮箱密码</th>'+
				'<th class="menuss" style="text-align: center; width :10%;">QQ号</th>'+
				'<th class="menuss" style="text-align: center;width :10%;" >QQ密码</th>'+
				'<th class="menuss" style="text-align: center ;	width :10%;">QQ群</th>'+
				'<th class="menuss" style="text-align: center ;	width :10%;">来源</th>'+
				'<th class="menuss" style="text-align: center ;	width :40%;">描述</th>'+
				'</tr>';
				...
				//	alert(item.dataSource)
					var html = 						
								...
								'<td id="menus" style="text-align:center; vertical-align:middle; width :10%;"  >'+item.emailPasswd+'</td>'+
								'<td id="menus" style="text-align:center; vertical-align:middle; width :10%;" >'+item.qqNum+'</td>'+
								'<td id="menus" style="text-align:center; vertical-align:middle; width :10%;"  >'+item.qqPasswd+'</td>'+
								'<td id="menus" style="text-align:center; vertical-align:middle; width :10%;">'+item.qunNum+'</td>'+
								'<td id="menus" style="text-align:center; vertical-align:middle; width :10%;">'+item.dataSource+'</td>';
								if(item.dataSource=="<font style='c11'>QQ</font>"||item.dataSource=="QQ"){
									html+=	'<td  id="menus" style="text-align:center; vertical-align:middle; width:40%">用户名:'+item.nick+'&nbsp;&nbsp;&nbsp;&nbsp;性别:'+item.gender+'&nbsp;&nbsp;&nbsp;&nbsp;群创建日期:'+item.createDate+'&nbsp;&nbsp;&nbsp;&nbsp;群标题:'+item.title+' &nbsp; &nbsp;&nbsp; &nbsp;群描述:'+item.qunText+' </td>'+'</tr>';
								}else if(item.dataSource=="<font style='c11'>JD</font>"||item.dataSource=="JD"){
									html+='<td id="menus" style="text-align:center; vertical-align:middle; width :40%;">用户名:'+item.nick+'&nbsp;&nbsp;&nbsp;&nbsp;真名:'+item.realName+'&nbsp;&nbsp; &nbsp;&nbsp;身份证号:'+item.ID+'&nbsp;&nbsp;&nbsp;&nbsp;手机号:'+item.telephone+' &nbsp;&nbsp;&nbsp;&nbsp;创建日期:'+item.createDate+' </td>'+'</tr>';
								}else if(item.dataSource=="<font style='c11'>网易</font>"||item.dataSource=="网易"){
									html+='<td id="menus" style="text-align:center; vertical-align:middle; width :40%;">用户名:'+item.nick+'&nbsp;&nbsp;&nbsp;&nbsp;真名:'+item.realName+'&nbsp;&nbsp; &nbsp;&nbsp; </td>'+'</tr>';
								}else if(item.dataSource=="<font style='c11'>腾讯</font>"||item.dataSource=="腾讯"){
									html+='<td id="menus" style="text-align:center; vertical-align:middle; width :40%;">用户名:'+item.nick+'&nbsp;&nbsp;&nbsp;&nbsp;日期:'+item.createDate+' </td>'+'</tr>';
								}else if(item.dataSource=="<font style='c11'>百度云</font>"||item.dataSource=="百度云"){
									html+='<td id="menus" style="text-align:center; vertical-align:middle; width :40%;">用户名:'+item.nick+'&nbsp;&nbsp;&nbsp;&nbsp;日期:'+item.createDate+' </td>'+'</tr>';
								}else if(item.dataSource=="<font style='c11'>珍爱网</font>"||item.dataSource=="珍爱网"){
									html+='<td id="menus" style="text-align:center; vertical-align:middle; width :40%;">用户名:'+item.nick+'&nbsp;&nbsp;&nbsp;&nbsp;日期:'+item.createDate+' </td>'+'</tr>';
								}else if(item.dataSource=="<font style='c11'>新浪</font>"||item.dataSource=="新浪"){
									html+='<td id="menus" style="text-align:center; vertical-align:middle; width :40%;">用户名:'+item.nick+'&nbsp;&nbsp;&nbsp;&nbsp;日期:'+item.createDate+' </td>'+'</tr>';
								}else{
									html+='<td  id="menus" style="text-align:center; width:50%;">创建日期:'+item.createDate+'</td>'+'</tr>';
								}
