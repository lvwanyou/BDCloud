...
<body>
				...
				<div class="panel panel-default">
					...
					<input type="hidden" id=evName value="<%=evi%>">
						<table id="datatable" class="table table-striped table-hover br04"
							...
							<thead>
								<tr>
									<th class="alcenter"  style="text-align:left;padding-left: 35px;">案件编号</th>
							...
							<tbody id="tbcont">
								<tr data-toggle="modal" data-target="#myModal" style="text-align: left;">
									<td style="text-align:left;padding-left: 35px;"><%=caseinfo.getCaseName()%></td>
									<td><%=caseinfo.getCaseNum()%></td>
									<td><%=caseinfo.getAcceptTime()%></td>
									<td><%=caseCity%></td>
	...
	<!-- 提交按钮 -->
		<div style="text-align: center;padding: 25px;">
                <button type="button" class="btn w-xs b31 c02" disabled="dasd" id="tijiao" style="margin-right: 30px;" onclick="tiaozhuan()">成功</button>
...
/* 	setInterval(function(){
	...
	function tiaozhuan(){
		...
		window.location.href= "<%=basePath%>noCaseData/getevidencelist2.php?uploadNum="+getUploadNum+"&successNum="+getSuccessNum+"&errorNum="+getErrorNum+"&evSize="+daxiao;
<%-- 		$.ajax({
			type : "POST",
			url : "<%=basePath%>noCaseData/getevidencelist2.php",
			data : {
			...
			success : function(data) {
				if (data.res == "succ") {
					...
					window.location.href= "<%=basePath%>admin/evidence_adding.php?evid=data."; 
				}
			},
	...
	function update(){
		...
		$.ajax({
			type : "POST",
			url : "<%=basePath%>noCaseData/updateEvidence2.php",
			data : {
				"uploadNum":getUploadNum,
				"successNum":getSuccessNum,
				"errorNum":getErrorNum,
