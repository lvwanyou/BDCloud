...
//	var uName11 = document.getElementById('userrole_right').innerHTML;     //获取案件负责人
//	var uName11 = document.getElementsByName("userrole_right").innerHTML;
//	alert(uName11);
//	var username = '${user.username}';//获取当前登陆用户  privilege
	var userPrivilege = '${user.privilege}';//获取当前登陆用户的职位   privilege
	...
	$.ajax({
        ...
        var personFlags = data.personFlags;
 //       alert(data.personFlags);
      ...
      //禁止授权
    	if(personFlags == 1){
           ...
           $("#id_right").html(data1.id);
           $("#caseNum_right").html(data1.caseNum);
           $("#caseName_right").html(data1.caseName);
           $("#caseType_rignt").html(data1.caseType);	//案件类型	userrole_right
           
           if(data1.shi == "市辖区" || data1.shi == "市辖县"){
        	   $("#shi_right").html(data1.sheng);				//案件所在城市
           } else {
           	   $("#shi_right").html(data1.sheng + data1.shi);				//案件所在城市
           }
           $("#section_right").html(data1.section);
           $("#trustee_right").html(data1.trustee);
           $("#status_right").html(data1.status);
           $("#label_right").empty();
           var getLabel = data1.label;
           var getEveryLabel;
           if(getLabel.length != 0) {
        	   ...
        	   for(i=0;i<getEveryLabel.length;i++) {
            	   ...
            	   var s = '<span>'+getEveryLabel[i]+'</span>';
       			   $("#label_right").append("<span class='br05 b12 span-1'>"+s+"</span>");
            	   }
            	   if(i % 7 == 1) {
            	   var s = '<span>'+getEveryLabel[i]+'</span>';
       			   $("#label_right").append("<span class='b13 br06 span-2'>"+s+"</span>");
            	   }
            	   if(i % 7 == 2) {
            	   var s = '<span>'+getEveryLabel[i]+'</span>';
       			   $("#label_right").append("<span class='br11 b11 span-3'>"+s+"</span>");
            	   }
            	   if(i % 7 == 3) {
            	   var s = '<span>'+getEveryLabel[i]+'</span>';
       			   $("#label_right").append("<span class='br07 b14 span-4'>"+s+"</span>");
            	   }
            	   if(i % 7 == 4) {
            	   var s = '<span>'+getEveryLabel[i]+'</span>';
       			   $("#label_right").append("<span class='br08 b15 span-5'>"+s+"</span>");
            	   }
            	   if(i % 7 == 5) {
            	   var s = '<span>'+getEveryLabel[i]+'</span>';
       			   $("#label_right").append("<span class='br09 b17 span-6'>"+s+"</span>");
            	   }
            	   if(i % 7 == 6) {
            	   var s = '<span>'+getEveryLabel[i]+'</span>';
       			   $("#label_right").append("<span class='br10  b18 span-7'>"+s+"</span>");
            	   }
            	   if(i % 7 == 7) {
            	   var s = '<span>'+getEveryLabel[i]+'</span>';
       			   $("#label_right").append("<span class='br11 b11 span-3'>"+s+"</span>");
           ...
           $("#createdTime_right").html(data1.createdTime);
           $("#accepted_date").html(data1.acceptTime);	//案件受理日期 
           $("#trustee_right1").html(data1.userName);	//案件受理人
           
           $("#relatedObject_right").html(data1.relatedObject);  //涉案物品  
           
           $("#list_companies").html(data1.relatedCompany);	//涉案公司列表
           
           //涉案人员
          $("#mainParty").empty();
           var mainPartyhtml="";
           var mainPartystrs = data1.mainParty.split("/");
...
function youjian() {
	var caseid;
	caseid = $("#id_right").text();
	var casename = $("#caseName_right").text();
	var casename2 = encodeURI(encodeURI(casename));
	window.location.href = "<%=basePath %>admin/youjian.php?caseid="+caseid+"&casename="+casename2;
}
...
function huadan() {
	...
	var casename = $("#caseName_right").text();
	var casename2 = encodeURI(encodeURI(casename));
	window.location.href = "<%=basePath %>admin/huadan.php?caseid="+caseid+"&allphone="+allphone+"&casename="+casename2;
}
...
function showAllSection(){
	$.ajax({
		...
		success : function(data) {
			...
			$("#section").append('<option class="c21" value="">请选择...</option>');
			...
			url:geturl,
					...
					success : function(data) {
						...
						if (pagesum < sizes) {
							...
							$("#casepages1").empty();
							$("#casepages2").empty();
//							$("#casepagesToPages").empty();
//							$("#butConfirmAAA").empty();
							
							var html3 = '<span >共' + pagesum + '条，当前' + pageno	+ '/' + pagenum + '页</span>';
							$("#casetot").append(html3);
							
						/* 	var html66 = '<input class="form-control" type="text" style="width:45px;height:25px;border-radius:2px;" id="givePages" name="givePages" onkeydown="onKeyDown(event)"/>';
							$("#casepagesToPages").append(html66); */
							
							//var peoplesGivePages = $("#givePages").val();
								//$("#givePages").val();
							//alert(peoplesGivePages);
							//var butConfirm = '<button type="button" class="btn btn-info" onclick="getCases1('+peoplesGivePages+')" style="width:52px;height:25px;background-color:#475E9C;color:#fff;">确定</button>';
						//	$("#butConfirmAAA").append(butConfirm);
							...
							$("#datatable_tou").empty(); 
							var htmltou = '<tr>'
									+ '<th style="text-align:left;">案件编号</th>'
									+ '<th style="text-align:left;">案件名称</th>'
									+ '<th style="text-align:left;">案件受理日期</th>'
									+ '<th style="text-align:left;">案件类型</th>'
									+ '<th style="text-align:left;">所属科室</th>'
									+ '<th style="text-align:left;">案件负责人</th>'
							...
							$.each(items,function(i, item) {
												var html = '<tr data-toggle="modal" data-target="#myModal" onclick="searchone(this)">'
														+ '<td style="display: none;"style="text-align:left;" >'
														+ item.id
														+ '</td>'
														+ '<td class="breakLine" style="text-align:left;">'
														+ item.caseNum
														+ '</td>'
														+ '<td class="breakLine" style="text-align:left;">'
														+ item.caseName
							...
							$("#caseType").empty();							
							  var htmla='<option class="c21" value="" >请选择...</option>';
							...
							$.each(caseType_items,function(i, item) {
								...
								$("#section").empty();		
							  var htmlc='<option class="c21" value="">请选择...</option>';
	...
	$.ajax({
		...
		success : function(data) {
			...
			for(var i=0;i<data.resData.length;i++){			
				...
				$("#label1").append($("<span name='shanchu' class='br05 b12 span-1' id='labelInfo' style='align:center;width: 70px; height: 30px; text-align: center;margin-right: 20px;margin-top: 10px;'>"+s+"</span>")); */
				
				var s = '<span id="label_'+data.resData[i].id+'" class="tags" title="'+data.resData[i].label+'" >'+data.resData[i].label+'</span>';
				if(i % 7 == 1) {
					$("#label1").append("<span name='shanchu' class='br05 b12 span-1' id='labelInfo"+data.resData[i].id+"' onclick='changebg("+data.resData[i].id+",\""+data.resData[i].label+"\")'>"+s+"</span>");
				}
				
				if(i % 7 == 2) {
					$("#label1").append("<span name='shanchu' class='b13 br06 span-2' id='labelInfo"+data.resData[i].id+"' onclick='changebg("+data.resData[i].id+",\""+data.resData[i].label+"\")'>"+s+"</span>");
				}
				
				if(i % 7 == 3) {
					$("#label1").append("<span name='shanchu' class='br11 b11 span-3' id='labelInfo"+data.resData[i].id+"' onclick='changebg("+data.resData[i].id+",\""+data.resData[i].label+"\")'>"+s+"</span>");
				}
				if(i % 7 == 4) {
					$("#label1").append("<span name='shanchu' class='br07 b14 span-4' id='labelInfo"+data.resData[i].id+"' onclick='changebg("+data.resData[i].id+",\""+data.resData[i].label+"\")'>"+s+"</span>");
				}
				if(i % 7 == 5) {
					$("#label1").append("<span name='shanchu' class='br08 b15 span-5' id='labelInfo"+data.resData[i].id+"' onclick='changebg("+data.resData[i].id+",\""+data.resData[i].label+"\")'>"+s+"</span>");
				}
				if(i % 7 == 6) {
					$("#label1").append("<span name='shanchu' class='br09 b17 span-6' id='labelInfo"+data.resData[i].id+"' onclick='changebg("+data.resData[i].id+",\""+data.resData[i].label+"\")'>"+s+"</span>");
				}
				if(i % 7 == 7) {
					$("#label1").append("<span name='shanchu' class='br10  b18 span-7' id='labelInfo"+data.resData[i].id+"' onclick='changebg("+data.resData[i].id+",\""+data.resData[i].label+"\")'>"+s+"</span>");
				}
				if(i % 7 == 0) {
					$("#label1").append("<span name='shanchu' class='br11 b11 span-3' id='labelInfo"+data.resData[i].id+"' onclick='changebg("+data.resData[i].id+",\""+data.resData[i].label+"\")'>"+s+"</span>");
				}
					...
					<div class="panel-body" id="searchOfForm" style="display: block;">
						<form class="form-inline" role="form" id="searchForm" name="form1"
							...
							<div class="form-group" style="width: 22%;">
								<label for="" class="clabel">案件编号:</label><input
									class="addWid form-control"
									style="width: 60%; border-radius: 2px;" id="caseNum"
									name="caseNum" value="${caseNum }" class="form-control"
									placeholder="请输入..." type="text" onkeydown="onKeyDown(event)" />
							</div>
							<div class="form-group" style="width: 22%; overflow: hidden;">
								<label for="" class="clabel">案件名称:</label><input
									class="addWid form-control"
									style="width: 60%; border-radius: 2px; overflow: hidden;"
									id="caseName" name="caseName" value="${caseName }"
									class="form-control" placeholder="请输入..." type="text"
									onkeydown="onKeyDown(event)" />
							</div>
							<%-- <div class="form-group" style="width: 22%;">
								<label for="" class="clabel">案件状态:</label> <select
									class="form-control addWid c21"
									style=" width: 60%; border-radius: 2px;"
									id="status" name="status" onkeydown="onKeyDown(event)">
							...
							<div id="hidden" style="display: none;">
								...
								<div class="form-group" style="width: 22%">
									<label for="" class="clabel">案件类型:</label> <select
										class="form-control c21"
										style="width: 60%;margin-left: -4px;"
										id="caseType" name="caseType" onkeydown="onKeyDown(event)">

										<option class="c21" value="" >请选择...</option>

										<c:forEach items="${caseTypeList }" var="cType">
											<option style="" id="caseTypes" value="${cType.typeName}"
								...
								<div class="form-group" style="width: 22%; display: none;" id="Sectionstype">
									<label for="" class="clabel">所属科室:</label> <select
										class="form-control c21"
										style="width: 60%; margin-left: -4px;"
										id="section" name="section" onkeydown="onKeyDown(event)">
										<option class="c21" value="">请选择...</option>
								...
								<div class="form-group" style="width: 22%">
									<label for="" class="clabel">所在地区:</label>
									<input id="shi" name="shi" value="${shi }" class="form-control"
										placeholder="请输入..." type="text" style="width: 60%; margin-left: 4px;"
										onkeydown="onKeyDown(event)" />
								</div>
								<br>
								...
								<div class="form-group" style="margin-top: 20px;">
									<label for="" class="clabel">&nbsp;&nbsp;&nbsp;至</label> <input
										...
										class="form-control" placeholder="请输入结束时间" type="text" style="width: 168px;"
								...
								<div class="form-group">
									...
									<!-- 页码 -->
									<div style="text-align:left;" style="font-size: 14px">
										<div class="pagecount inline col-md-offset-4" style="height: 29px; margin-top: 5px"></div>
										<div class="pagecount inline col-md-offset-4" style="position: absolute; height: 29px; right: 300px; margin-top: 5px">
											<span id="tot_label"></span>
										</div>
										<div class="pagebar inline" style="position: absolute; right: 30px; height: 29px;">
											<ul class="pagination pagination-sm" style="margin: 0;">
												<li id="pages1_label"><a></a></li>

												<li id="pages_label"><a></a></li>

												<li id="pages2_label"><a></a></li>
											</ul>
										</div>
									</div>
									<!-- 页码 ——end-->
					...
					<div class="panel-body" style="padding: 0 0 15px;"> 
						...
						<div style="text-align:center;" style="font-size: 14px;">
							<div class="pagecount inline" id="casetotal" style="height: 29px;">
								<span id="casetot" style="float: left;"> </span>
							...
							<div style="float: right; margin-right: 11px;">
								<input class="form-control" type="text" style="width: 52px; height: 28px; border-radius: 2px; display: inline;"
									id="givePages" name="givePages" onkeydown="onKeyDown(event)" />
								<button type="button" class="btn" onclick="getCases1(1)" style="width: 52px; height: 28px; line-height: 12px;">跳转</button>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="modal-content">
					<div class="panel panel-default" style="padding-right: 10px;">
						<div class="panel-body" style="overflow-y: auto; height: 750px;">
							<div class="panel-heading br004 b22"
								style="padding: 10px;">案件信息</div>
							<table id="datatable" class="table table-striped table-hover br004">
								<tbody id="tbcont">
									...
									<!-- <tr data-toggle="modal" style="">
										<td class="td_left">userrole</td>
										<td class="td_right" id="userrole_right" name="userrole_right"></td>
									</tr> -->
							...
							<div class="panel-heading b22 br004"
								style="padding: 10px;">涉案组织</div>
							<table id="datatable" class="table table-striped table-hover br004">
							...
							<div class="panel-heading b22 br004"
								style="padding: 10px;">涉案人员</div>
							<table id="datatable" class="table table-striped table-hover br004"
								style="text-align: center;">
								<thead>
									<tr>
										<th style="text-align:left;width:25%">嫌疑人姓名</th>
										<th style="text-align:left;width:25%">位置</th>
										<th style="text-align:left;width:25%">手机号码</th>
										<th style="text-align:left;width:25%">电子邮箱</th>
									</tr>
								</thead>
								<tbody id="mainParty">
							...
							<div class="panel-heading b22 br004" style="padding: 10px;">涉案物品</div>
							<table id="datatable" class="table table-striped table-hover br004">
					...
					<div style="text-align: center;">
						<div class="">
							...
							<a href="<%=basePath%>admin/edit_case.php" class="btn" id="editCaseA"
								style="margin-right: 20px; background-color: #475e9c; color: #fff; width: 15%;">编辑案件</a>

							<a href="#" class="btn" onclick="getid();" id="editCaseB"
								style="margin-right: 20px; background-color: #475e9c; color: #fff; width: 15%;">数据列表</a>

							<a id="nextA" class="btn" href="<%=basePath%>admin/case_new_granttwo.php" onclick="return setHref();"
								style="margin-right: 20px; background-color: #475e9c; color: #fff; width: 15%;">案件授权</a>
							<a id="yj" class="btn" onclick="youjian();"
								style="margin-right: 20px; background-color: #475e9c; color: #fff; width: 15%;">邮件分析</a>
							<a id="hd" class="btn" onclick="huadan();"
								style="margin-right: 20px; background-color: #475e9c; color: #fff; width: 15%;">话单分析</a>
						</div>
					</div>
				</div>
			</div>
...
//			          案件编号联想
						$( "#caseNum" ).autocomplete({
							  source: function( request, response ) {
						          var matcher = new RegExp( "^" + $.ui.autocomplete.escapeRegex( request.term ), "i" );
						          response( $.grep( seNum, function( item ){
						              return matcher.test( ConvertPinyin(item) ) || matcher.test( item );
						          }) );
						      }
						});
	...
	//判断是否局长登录，显示所有科室案件选择						
	var username = '${user.privilege}';//获取当前登陆用户
	if(username=="局长"){
		$("#Sectionstype").css("display","inline-block");
	}
