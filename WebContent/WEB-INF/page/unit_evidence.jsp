	...
	<div class="wrapper-md">
		<div class="col-md-12" style="margin-bottom:27px;">
			...
			<div class="" style="text-align: left; margin-bottom: 20px; margin-top: 20px;">
				<a href="<%=basePath%>admin/importevidence_unit.php" type="button" class="btn w-xs btn-info" style="width: 135px;"
...
<%-- 									<td>${ev.evName }</td>
					...
					<%-- 				<td><c:if test="${ev.dataTypes == 1}">移动设备</c:if> <c:if
											...
											test="${ev.dataTypes == -1}">未选择数据来源</c:if></td> --%>
...
<%-- 									
									...
									<c:choose> 
     									<c:when test="${result.clstate==-1}">无法建立索引</c:when>      
		     									<c:otherwise>  <!--否则 --> 
		     										<c:if test="${result.clstate==2}">
														<div id="imghtml${ev.id }">
															<a href="#" onclick="addSolr('${ev.id }')">建立索引</a>
														</div>
														<td id="addIndex${ev.id }"><p></p></td>
													</c:if>
													<c:if test="${ev.indexFlag==1}">已建立索引</c:if>
													<c:if test="${ev.indexFlag==2}">索引建立失败</c:if> 
...
function subForm(pageno){
	...
	window.location.href = "<%=basePath%>admin/unitevidencelist.php?clueid="+clueid+"&pageno="+pageno;
}
...
function getcaseId() {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>admin/addunit.php",
		cache:false, 
	    async:false,  
		data : {
