...
function subForm(pageno){
	var givePages_cancel = $("#givePages_cancel").val();
	if(givePages_cancel != ""){
		pageno = parseInt(givePages_cancel);
	} 
	$("#pageno").val(pageno);
	$("#searchForm").submit();
}
...
function setHref(){
	var array=document.getElementsByName("ids");
   	...
   	var param = "id="+ids.toString()+"&noStr="+noArr.toString()+"&str="+str+"&unameStr="+unameArr.toString();
	param = encodeURI(param);
	param = encodeURI(param);
   	$("#nextA").attr("href", "<%=basePath%>admin/case_cancel_granttwo.php?"+param);
	return true;
}
//回车搜索事件  分页
function onKeyDown_cancel(event){
	...
	<div class="wrapper-md">
		<div class="col-md-12" style="margin-bottom:27px;">
			<div class="row">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<form class="form-inline" id="searchForm" action="<%=basePath %>admin/case_cancel_grant.php" method="post">
							...
							<div id="showChooseDiv" class="form-group" style="display: none;margin-left: 15px;">
								已选案件：<span class="btnA" id="showChooseSpan"></span>
							</div>
					...
					<div class="panel-body" style="padding: 0 0 15px;">
							...
							<tbody id="tbcont">
								<c:forEach items="${dataList }" var="item">
								 	<tr>
										...
										<td id="${item.id }_name">${item.userName }</td>
										<td id="${item.id }_uname">${item.trustee }</td>
									</tr>
								 </c:forEach>
							</tbody>
						</table>
						<div class="alcenter" style="font-size: 14px">
							...
							<div style="float: right;margin-right: 11px;">
								<input class="form-control" type="text" style="width:52px;height:28px;border-radius:2px; display: inline;" id="givePages_cancel" name="givePages_cancel" onkeydown="onKeyDown_cancel(event)"/>
			...
			<div align="center" style="margin-top: 34px;margin-bottom: 30px;">
				<a id="nextA" class="btn b23 c02" href="<%=basePath%>admin/case_cancel_granttwo.php" onclick="return setHref();"
		...
		<div class="modal fade" id="checkCase_tanchuan" tabindex="-1" role="dialog"
			...
			<div class="modal-dialog" style="width: 476px;">
				<div class="modal-content">
					...
					<div class="modal-body">请选择取消授权的案件！</div>
