...
<!-- 	<script type="text/javascript" src="http://api.map.baidu.com/api?v=2.0&ak=4aflybZEtwQXleKeqP4almEW1wH1Tqv6"></script> -->
<!--离线地图加载  -->
...
<body  style="overflow-y:scroll;">
	...
	<div class="row" style="height:; width: 98%; margin-left: 1%; margin-top: 1%;">
		<div class="form-group" style="float: left;margin-bottom:0px;">
			...
			<div id="spans" style="margin-left: 75px; overflow:hidden;"></div>
		</div>
			<div>
			...
			<div class="row">
					...
					<div class="panel-body" id="searchOfForm" style="display: block;">
						<form class="form-inline" role="form" id="searchForm" action="<%=basePath %>admin/case_xietong.php" method="post">
							...
							<div class="form-group">
								...
								<input id="dirName" type="text" class="form-control" placeholder="请输入图片名称..." onkeydown="onKeyDown(event)"/>
							</div>
							<button type="button" class="btn btn-info c25 c02"
				...
				<img id="pic_one2" src="" style="vertical-align: middle; width: 100%;" onclick="setDisplay()">
					...
					<div class="alcenter" style="font-size: 14px;margin-bottom:10px;display:none;">
						<div id="total_GPS" class="pagecount inline" style="height: 29px;">
							<span id="tot1_GPS"></span>
						</div>
...
function showPicture(pageno){
		...
		$.ajax({
			type : "POST",
			url : "<%=basePath%>admin/showPictureInfo_GPS.php",
			data : {
				"pageIndex":beipage,
				"dirName":dirName,
                "caseidStr":caseidStr
			...
			success : function(data) {
				//alert(data.total);//var docs=data.hits.hits;
				var sizes=10;
				...
				for(var i=1;i<=length;i++)
					  $("#orderA_GPS").remove();
				if(pagesum<sizes){
					var html2 = '<li class="active" id="orderA_GPS"><a href="#" onclick="showPicture(1)">1</a></li >';
					$("#pages_GPS").after(html2);	
				}else{
				if(pageno<pagenum){
							...
							for(var i =pageno-2;i<=pageno+length-1-2;i++){	
								if(i==pageno)
									html2 += '<li class="active" id="orderA_GPS"><a href="#" onclick="showPicture('+i+')">'+i+'</a></li >';
										...
										html2 += '<li id="orderA_GPS"><a href="#" onclick="showPicture('+i+')">'+i+'</a></li >';  
						   				   }
							}
						else{
							for(var i =1;i<=length;i++){	
				...
				var imgs=data.piclist;
				  $.each(imgs,function(i,item){
					 ...
					 var html =   '<div class="small_pic js-wfl-elem absolute" >'+
				                	 '<a href="#pic_one2"class="addHov br17" style="display:inline-block;position:relative;height:auto;width:100%;line-height:198px;"><img onload="loadcomplete()" id="picUrl'+i+'" src="/picture/'+img2+'"style="height: auto;width:100%; " onclick="getPicUrl('+i+')" />'+'<span class="addFot c02">'+picNam +'</span>'+'</a>'+
				                	'<span id="pictureSrc'+i+'" style=" display:none;">'+item.picDirpath+'</span>'+
				                	'<span id="pictureStarFlag'+i+'" style=" display:none;">'+item.starFlag+'</span>'+
	...
	function getPicUrl(i) {
		...
		$.ajax({
			type: "POST",
			url: "<%=basePath %>admin/pictureGPS.php",
			dataType : "json",
			data : {
				"pUrl" : pUrl
			},						
			success : function(data) {
				$("#info_pic").empty();	
/* 				data.latitude=39.899855;
				data.longitude=116.259307;  北京经纬度*/
			/* 	if(data.latitude===undefine){
					alert(1)
				} */
			/* 	alert(data.latitude+":"+data.longitude); */
				if (typeof(data.latitude) != "undefined"&&typeof(data.longitude) != "undefined") {  // 判断是否得到图片src	
					if(data.latitude!=0.0&&data.longitude!=0){	//判断图片是否具有lat,lon
					//alert(data.latitude);
					document.getElementById("container").style.display="block";	//加载地理位置展示
					showPicGPS(data.latitude,data.longitude);	
					}else{
						alert("该图片无经纬度信息！");
						document.getElementById("container").style.display="none";	//加载地理位置展示
					}	
				}			
	...
	//选择某个复选框案件将案件信息展示在页面上
		function chooseCase(id){
		 ...
		 if(caseInfo.checked){
			...
			var html ='<span class="br05 b12 span-1" id="shanchu2'+caseId+'" style="width: 150px; height: 30px;margin-right: 15px;margin-top: 5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" onclick="deletemainParty(this.id,'+caseId+')">'+caseName+
...
//点击地名显示对应地点
function showPicGPS(lat,lon){
	// 百度地图API功能，显示离线地图
	var map = new BMap.Map("container", {minZoom:2,maxZoom:8});    // 创建Map实例
	  ...
	  //坐标转换完之后的回调函数
	    translateCallback = function (data){
	      if(data.status === 0) {
	        ...
	        var label = new BMap.Label("目标位置",{offset:new BMap.Size(20,-10)});
		...
		//设置图片的显示
		$("#gpsPicImg").attr("src","<%=basePath%>preview/"+imgpath);
	} --%>
	...
	var _wfl = {
	    ...
	    wfl_fomat:'<div id="%id%" class="js-wfl-elem small_pic absolute">'+
					    '<a href="#pic_one2" class="addHov br17" style="display:inline-block;position:relative;height:auto;line-height:198px;">'+
							'<img onload="loadcomplete()" src="%picpath%" id="picUrl%idi%" style=" height: auto;width:100%;" onclick="getPicUrl(%idi%)"/>'+
							'<span class="addFot c02">%picNam%</span>'+
						'</a>'+
						'<span id="pictureSrc%idi%" style=" display:none;">%pictureSrc%</span>'+
