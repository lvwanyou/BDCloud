		...
		<div class="col-md-12" style="margin-bottom:27px;">
			<div class="row">
				<div class="panel panel-default">
					<div class="panel-heading">搜索日志<span style="float: right;"><img src="<%=basePath %>template/img/addevidence.png" onclick="testOfSearch(this)"></span></div>
					<div class="panel-body" id="fold" style="display: block;">
						<form class="form-inline" role="form" id="searchForm" action="<%=basePath %>admin/case_xietong.php" method="post">
							<input type="hidden" name="pageno" id="pageno"/>
							<div class="form-group" style="width:25%">
								<label for="" class="c06 clabel">案件编号:</label><input id="caseNum" style="width:60%" name="caseNum" value="${caseNum }"
									class="form-control" placeholder="请输入..." type="text" onkeydown="onKeyDown(event)"/>
							</div>
							<div class="form-group" style="width:25%">
								<label for="" class="c06 clabel">案件名称:</label><input id="caseName" style="width:60%" name="caseName" value="${caseName }"
									class="form-control" placeholder="请输入..." type="text" onkeydown="onKeyDown(event)"/>
							</div>
							<div class="form-group" style="width:25%">
								<label for="" class="c06 clabel">被授权人:</label><input id="grantedName" style="width:60%" name="grantedName" value="${grantedName }"
									class="form-control" placeholder="请输入..." type="text" onkeydown="onKeyDown(event)"/>
							</div>
							<button type="button" class="btn btn-info b23 c02" id="js-point"
				...
				<div class="panel panel-default">
					...
					<div class="panel-body" style="padding: 0 0 15px;">
						<table id="datatable" class="table table-striped table-hover br04"
							...
							<thead>
								<tr>
									<th class="c20" style="color:#58666e;text-align:left;">操作日期</th>
									<th class="c20" text-align:left;">操作类型</th>
									<th class="c20" text-align:left;">案件编号</th>
									<th class="c20" text-align:left;">案件名称</th>
									<th class="c20" text-align:left;">案件类型</th>
									<th class="c20" text-align:left;">所属部门</th>
									<th class="c20" text-align:left;">授权人姓名</th>
									<th class="c20" text-align:left;">被授权人姓名</th>
									<th class="c20" text-align:left;">被授权人职务</th>
									<th class="c20" text-align:left;">所在科室</th>
								</tr>
							</thead>
							<tbody id="tbcont">
								 <c:forEach items="${logs }" var="item">
								 	<tr>
										<td style="text-align:left;">${item.operateTime }</td>
										<td style="text-align:left;">${item.type }</td>
										<td style="text-align:left;">${item.caseNum }</td>
										<td style="text-align:left;">${item.caseName }</td>
										<td style="text-align:left;">${item.caseType }</td>
										<td style="text-align:left;">${item.department }</td>
										<td style="text-align:left;">${item.grantName }</td>
										<td style="text-align:left;">${item.grantedName }</td>
										<td style="text-align:left;">${item.grantedPri }</td>
						...
						<%-- <div style="font-size: 14px;text-align: center;">
							...
							<div style="float: right;margin-right: 11px;">
								<input class="form-control" type="text" style="width:52px;height:28px;border-radius:2px; display: inline;" id="givePages_xietongs1" name="givePages_xietongs1" onkeydown="onKeyDown_xietong(event)"/>
			...
			<div align="center" style="margin-top: 34px;margin-bottom: 30px;">
				<a href="<%=basePath%>admin/case_new_grant.php"><button class="btn w-xs btn-info" style="margin-right: 20px;width: 135px;">新增授权</button></a>
                <a href="<%=basePath%>admin/case_cancel_grant.php"><button class="btn w-xs btn-info" style="width: 135px;">取消授权</button></a>
			</div>
		</div>
...
function subForm(pageno){
	...
	var grantedName=$("#grantedName").val();
	if(caseNum!=null&&caseNum!=""||caseName!=null&&caseName!=""||grantedName!=""&&grantedName!=null){
		$("#searchForm").submit();
		return true;
	}else{
...
function testOfSearch(obj){
  var div1=document.getElementById("fold");
